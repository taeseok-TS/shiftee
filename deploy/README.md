...
업데이트 6단계 뒤에 자동으로 붙는다(`--skip-smoke` 로 끌 수 있다). 따로 돌릴 수도 있다.

```bash
/opt/qubetee/deploy/smoke-test.sh --direct        # 직영
/opt/qubetee/deploy/smoke-test.sh --code acme     # 고객사
```

**읽기만 한다.** 운영 데이터를 만들지도 지우지도 않고, 비밀번호도 필요 없다
(컨테이너 안에서 관리자 토큰을 직접 서명해 쓴다).

28개 항목: 인증 차단·로그인 거부 / 직원·지점·대시보드·현황·출퇴근·근무일정·휴가·연차·결재선·
계약·템플릿·공휴일·제안·메신저 API / 주요 페이지 5개 / 응답 형태 / **DB↔스키마 차이 0** / 최근 오류 로그.

`/login` 200 만 보는 헬스체크는 "서버가 살아있다"까지만 증명한다 —
그 뒤 화면들이 500 을 내도 통과한다. 그 간극을 메우는 것이 이 점검이다.

**고장을 실제로 잡는지 확인했다(2026-08-18)**: DB 에 컬럼을 하나 추가하니 스키마 차이 1건으로 즉시 실패,
DB 컨테이너를 내리니 로그인 500 으로 즉시 실패. 복구 후 28개 전부 통과.
...
서버 안의 백업은 서버가 죽으면 같이 사라진다. `sync-backups.ps1` 이 디렉터 PC 로 매일 한 벌 더 받는다.

- 작업 스케줄러 `큐브티 백업 동기화` — 매일 10:00. PC 가 꺼져 있던 날은 다음 실행 때 만회된다
  (최근 며칠치를 통째로 다시 받고, 서버는 14일 보관하므로).
- 받는 것: 직영 DB 덤프 + 고객사별 DB 덤프 + 제거된 고객사의 마지막 백업. 전부 합쳐 2MB 안팎이다.
- 받은 뒤 **gzip 을 끝까지 풀어보며 무결성을 확인**한다. 덜 받은 파일은 다음 회차에 다시 받는다.
- 보관 위치 `문서\큐브티백업\` (직영 / 고객사\<코드> / 제거된고객사), 60일 보관.

**uploads(계약서 서명본·채팅 파일, 약 1GB)는 아직 오프사이트가 아니다.** 용량이 커서 매일 받기 어렵고,
현재는 Vultr 스냅샷에만 의존한다. 판매 규모가 커지면 오브젝트 스토리지를 검토할 것.
