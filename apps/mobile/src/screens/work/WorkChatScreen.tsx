...
export default function WorkChatScreen() {
  ...
  const addPending = (items: PendingAtt[]) => {
    ...
    // 사진 10장을 **먼저** 자르고 총량 20을 나중에 본다 — 순서를 바꾸면 자리가 남는데도 덜 담기고 안내가 두 번 뜬다(검증관 4)
    const msgs: string[] = [];
    if (merged.filter((m) => m.kind === "image").length > 10) {
      msgs.push("사진은 한 번에 최대 10장까지 첨부할 수 있습니다.");
      let n = 0;
      merged = merged.filter((m) => m.kind !== "image" || ++n <= 10);
    }
    if (merged.length > 20) {
      msgs.push("첨부는 한 번에 20개까지입니다.");
      merged = merged.slice(0, 20);
    }
    if (msgs.length) Alert.alert("알림", msgs.join(String.fromCharCode(10)));
