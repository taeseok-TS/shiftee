...
- 한도: 키당 분당 60회·하루 2,000회, 쓰기 분당 10회, 파일 올리기 하루 50개. 넘으면 429.
...
파일 내려받기: \`GET ${getAppUrl()}<files[].url>\` 에 같은 Authorization 헤더.
...
- 1시간에 500회를 넘으면 이상 사용으로 보고 키를 멈추고 본인·본부에 알립니다.
`;
