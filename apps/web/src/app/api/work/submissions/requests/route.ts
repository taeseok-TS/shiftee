...
import { isTargeted, requestsTargetingWhere, targetUsersFor } from "@/lib/submission-targets";
import { pickBranches, pickJobGroups, serializeRequest } from "@/lib/submission-server";
...
export async function GET(request: NextRequest) {
  ...
  if (manage) {
    const rows = await prisma.submissionRequest.findMany({
      where: {
        ...statusWhere,
        // 원장: 담당 지점이 대상에 들어가는 요청만(전 지점 요청 포함)
    ...
    return NextResponse.json({
      requests: rows.map((r) => {
        const targets = pool.filter((u) => isTargeted({ role: "EMPLOYEE", branch: u.branch, jobGroup: u.jobGroup }, r));
        const done = subBy.get(r.id) ?? new Set<string>();
        const submitted = targets.filter((t) => done.has(t.id)).length;
        return { ...serializeRequest(r), targetCount: targets.length, submittedCount: submitted };
      }),
    });
  }
...
export async function POST(request: NextRequest) {
  ...
  const row = await prisma.submissionRequest.create({
    data: { title, description: description || null, categoryId: category.id, targetJobGroups, targetBranches, dueDate, createdBy: session.userId, createdByName: session.name },
    include: { category: true },
  });
  await logAudit({
    ...
    detail: `${category.name} · 대상 ${targetJobGroups.length ? targetJobGroups.join("/") : "전 직군"} · ${targetBranches.length ? targetBranches.join("/") : "전 지점"} · 마감 ${body.dueDate || "없음"}`,
  });
  // 알림은 응답을 막지 않는다
