...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ docNo: string }> }) {
  ...
  const c = await prisma.contract.findUnique({ where: { docNo }, select: { status: true, tsaToken: true } });
  if (!c || c.status !== "SIGNED" || !c.tsaToken) return NextResponse.json({ found: false }, { status: 404 });
